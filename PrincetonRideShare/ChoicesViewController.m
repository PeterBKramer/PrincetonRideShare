...
//            [twitterOrFacebookWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"https://m.twitter.com"]]];
        ...
        aboutText.text=@"     This page allows you to compose a Message to send to another user and shows Messages that you have sent or received.\n\n     Before you can send or receive Messages you must read and agree to the conditions in a Disclaimer statement.  All Messages are sent to and received from anonymous users identified only by their iCloud Drive ID number which is a specific 7 digit random number assigned to every device using the same iCloud Account.  (This device's iCloud Drive ID number is displayed on the \"Intro\" page.)  Using these Messages you may determine that a particular anonymous user is a good prospect for carpooling and agree to meet or share non-anonymous identifying information to that user.  However, please read the safety information in the disclosure before revealing your identity.\n\n     You may not use this App to send Messages that are offensive.  If you receive a message that you feel is offensive, record the full 7 digit iCloud Drive ID of the sender and use the \"Contact Us\" button to send us an email describing the issue.\n\n     Messages to and from a particular user's devices (i.e. devices using a particular iCloud Account) are Grouped together.  Tap any Group to open it and display all of the Messages in that Group.  (There may be only one Message in a Group.)  Tap \"- Done\" to close that Group of Messages.\n\n     To send a Message to a matching user's devices (i.e. all devices using a particular iCloud Account associated with an iCloud Drive ID), tap the map annotation of a Match on the \"Match\" page and then tap \"Send Message\".  A standard 'first Message' text will appear in the text field and can be used as is, modified or deleted.   Alternatively, tap a Group to display its Messages and then tap any previous Message that you sent to, or received from, that user and a blank text field will appear.  Compose the new Message and tap \"Send\".  A Message that has not yet been retrieved by its recipient will display a blue arrow.  When a Message is retrieved by its recipient the arrow will turn grey.\n\n     To identify, on the map, all of the Rides of a sender or recipient of a Message, tap the Message and then tap \"Show Match\".  The annotation numbered \"1\" will be the specific matching Ride associated with the Message.\n\n     Each time you open this page the App goes out to iCloud and retrieves any new Messages.  A new Message just retrieved and unread will display a blue dot until its Group is opened and \"- Done\" is tapped or until that Message is tapped.  The number of such unread messages will be displayed in a red badge over the tab for this page.  While viewing this page you can refresh your Messages by tapping the circular refresh icon.\n\n     To delete any Message (or any Group) tap \"Delete Entries\" and then the red button and the \"Delete\" square.  Alternatively, swipe any Group or Message to the left and press the \"Delete\" square.  If you delete a Message that you sent before it is retrieved by its intended recipient, then that Message will no longer be retrievable by that recipient.\n\n     This App can send you Notifications that new Messages have been sent to you and are ready for retrieval.  To receive Notifications turn on the \"Alerts:\" switch.  (You may need to authorize this App to receive Notifcations on this device using your Settings App.)  All devices logged into the same iCloud Account share their alert Notifications.  With Notifications turned on (by any device using your iCloud Account), a red badge on the App Icon will indicate new Messages are awaiting retrieval.\n\n";
    }else if (theChoice==6){
        aboutText.hidden=NO;
...
-(void) showMailComposer{
    ...
    if(mailClass!=nil){
        if([mailClass canSendMail]){
            ...
            [picker setSubject:@"Feedback on Princeton Ride Share V1.3"];
